...
const CONTRACT = '0x867FC12E89606f7d55d92e5fcE26e1c67D3Af229'
...
function judge(url: string, ms: Milestone[]): Record<string, Verdict> {
  let h = 0
  const s = url + ms.length
  for (let i = 0; i < s.length; i++) h = (h * 33 + s.charCodeAt(i)) >>> 0
  const out: Record<string, Verdict> = {}
  ms.forEach((m, i) => {
    out[m.id] = (h >> i) % 5 === 0 ? 'fail' : 'pass'
  })
  return out
}
...
export default function App() {
  ...
  const [results, setResults] = useState<Record<string, Record<string, Verdict>>>({})
  const [phase, setPhase] = useState('')

  const active = useMemo(() => BOUNTIES.find((b) => b.id === activeId)!, [activeId])
  const activeResult = results[activeId]

  const scoreOf = (b: Bounty): number | null => {
    const r = results[b.id]
    if (!r) return null
    return b.milestones.reduce((acc, m) => acc + (r[m.id] === 'pass' ? m.weight : 0), 0)
  }

  useEffect(() => {
    if (!judging) return
    const steps = ['cloning repository…', 'reading deliverable…', 'matching milestone specs…', 'rendering verdict…']
    let i = 0
    setPhase(steps[0])
    const t = setInterval(() => {
      i++
  ...
  function runJudge() {
    if (!/github\.com\/.+\/.+/.test(url.trim())) {
      toast.error('Invalid GitHub URL', { description: 'e.g. https://github.com/org/repo' })
      return
    }
    setJudging(true)
    toast('Adjudication started', { description: `Fetching ${active.id} deliverable…` })
    setTimeout(() => {
      const verdicts = judge(url.trim(), active.milestones)
      setResults((prev) => ({ ...prev, [activeId]: verdicts }))
      setJudging(false)
      const passed = active.milestones.filter((m) => verdicts[m.id] === 'pass').length
      const total = active.milestones.length
      if (passed === total) toast.success('All milestones passed', { description: `${active.reward} USDC released in full.` })
      else if (passed === 0) toast.error('Bounty rejected', { description: 'No milestones met spec.' })
      else toast.warning(`${passed}/${total} milestones passed`, { description: 'Partial payout escrowed.' })
    }, 3000)
  }
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 flex gap-1">
                  {b.milestones.map((m) => {
                    const v = results[b.id]?.[m.id]
                    return (
                      <div
                        key={m.id}
                        className={`h-1.5 flex-1 rounded-full ${
                          v === 'pass' ? 'bg-emerald-500' : v === 'fail' ? 'bg-red-500' : 'bg-gray-700'
                        }`}
                      />
                    )
                  })}
          ...
          >
            ...
            <div className="p-6">
              <div className="flex items-center justify-between">
                ...
                {activeResult && (
                  <span className="font-mono text-xs font-bold text-[#FBBF24]">{scoreOf(active)}/100</span>
                )}
              </div>
              <div className="mt-4 space-y-3">
                {active.milestones.map((m, i) => {
                  const v = activeResult?.[m.id]
                  return (
                    <motion.div
                      key={m.id}
                      initial={false}
                      animate={{ opacity: 1 }}
                      className="flex items-center gap-3 rounded-xl border border-gray-800 bg-[#111827] p-3.5"
                    >
                      <motion.div
                        initial={false}
                        animate={
                      ...
                      <div className="shrink-0 text-right">
                        <div className="font-mono text-xs text-gray-400">{m.weight}%</div>
                        <div
              ...
              <AnimatePresence>
                {activeResult && (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className="mt-5 flex items-center justify-between rounded-xl border border-gray-800 bg-gradient-to-r from-[#FBBF24]/10 to-transparent p-4"
                  >
                    <div>
                      <p className="text-sm font-semibold text-gray-100">
                        Payout: {((scoreOf(active)! / 100) * active.reward).toFixed(0)} / {active.reward.toLocaleString()} USDC
                      </p>
                      <p className="font-mono text-[11px] text-gray-500">
                        Released proportional to milestones passed · escrowed on contract
                      </p>
                    </div>
                    <button
